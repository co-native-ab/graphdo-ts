...
Last updated: W2 Day 1 — Frontmatter codec
Current milestone: W2 Day 1 — Frontmatter codec (this PR)
Next milestone: W2 Day 2 — `doc_id` recovery + `frontmatter_reset` audit
...
| --------- | ---------------------------------------------- | --------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| W2 Day 1  | `copilot/collab-implementation-next-milestone` | _this PR_ | 2026-04-19 | Frontmatter codec landed. `yaml` added as a runtime dep pinned with `~2.8.0` (advisory-DB clean for `2.8.3`). New `src/collab/frontmatter.ts` owns the strict Zod schema for the §3.1 `collab:` block (version / doc_id / created_at / sections[] / proposals[] / authorship[] with `.strict()` everywhere and `.default([])` arrays), the hardened parser (`yaml.parseDocument` with `prettyErrors`, `strict`, `customTags: []`, `schema: "core"`, `logLevel: "silent"`; both `errors` **and** `warnings` escalated to `FrontmatterParseError` so unresolved tags fail loudly; pre-parse multi-doc `---`/`...` separator refusal; plain-`Object.prototype` root check; 256 KiB max-bytes guard), the deterministic emitter (canonical key-order rebuild, `defaultStringType: "QUOTE_DOUBLE"`, `indent: 2`, `lineWidth: 0`, `directives: false`, belt-and-braces in-emitter round-trip with `FrontmatterRoundtripError`), and the `splitFrontmatter` / `joinFrontmatter` envelope helpers (CRLF → LF normalisation on the way in, LF-only on the way out). New `test/collab/frontmatter.test.ts` (27 rows: round-trip determinism on minimal + populated + scrambled-key + Unicode inputs, strict-schema rejections at root / collab / proposal / version, malformed YAML, empty-body, sequence-root, oversize, custom-tag refusal, multi-doc refusal, `__proto__` smuggle refusal, envelope helpers on LF / CRLF / missing-closer / no-frontmatter / empty-body / split-then-rejoin) + `test/collab/frontmatter-snapshot.test.ts` (4 rows: byte-exact snapshot covering every §3.1 field including a Unicode display name, a rationale containing both `:` and `#`, and a collision-suffixed slug; LF-only / no-tabs guards). New `docs/adr/0008-frontmatter-codec.md`. `npm run check` green (792 → 823 tests, +31 new). Once merged, advance to W2 Day 2. |
...
- [x] **W2 Day 1** — Frontmatter codec _(this PR; ADR-0008 lands here: `yaml ~2.8.0` + deterministic emitter + byte-exact snapshot)_
