...
Last updated: W2 Day 4 — `collab_read` + `collab_list_files`
Current milestone: W2 Day 4 — `collab_read` + `collab_list_files` (this PR)
Next milestone: W2 Day 5 — buffer
...
- [x] **W2 Day 4** — `collab_read` + `collab_list_files` _(this PR; `src/tools/collab.ts` + scope wrapper + new graph helpers in `src/collab/graph.ts`; `test/integration/01-init-write-read-list.test.ts` + `test/integration/08-scope-traversal-rejected.test.ts` tool-layer rows lit up)_
- [ ] **W2 Day 5** — buffer
