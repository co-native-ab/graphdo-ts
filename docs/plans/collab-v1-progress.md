...
Last updated: W1 Day 5 — `session_status` + persisted destructive counter
Current milestone: W1 Day 5 — `session_status` + persisted destructive counter (this PR)
Next milestone: W2 Day 1 — Frontmatter codec (ADR-007 lands here)
...
| --------- | ---------------------------------------------- | --------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| W1 Day 5  | `copilot/collab-implementation-next-milestone` | _this PR_ | 2026-04-19 | `session_status` MCP tool + in-memory `SessionRegistry` (`src/collab/session.ts`) + destructive-counter persistence helper (`src/collab/session-counts.ts`, `<configDir>/sessions/destructive-counts.json`, §3.7 — strict Zod schema, atomic write, lazy-prune of entries with `expiresAt < now - 24h`). `ServerConfig.sessionRegistry` plumbed; `session_init_project` now refuses (`SessionAlreadyActiveError`) when a session already exists, then activates one with §5.2.1 defaults (TTL 2h, write 50, destructive 10) on success. `agentId` derived as `<oidPrefix>-<clientSlug>-<sessionIdPrefix>`; `clientSlug` hard-coded to `"unknown"` until W5 Day 3 plumbs `clientInfo.name`. `NoActiveSessionError` + `SessionAlreadyActiveError` added (live next to the registry). `session_status` reports `expired: true` (not `isError`) when past TTL. New `test/collab/session.test.ts` (slugify / agentId / persistence round-trip / pruning / start / clamping / increments / renew / end / NoActiveSessionError) and `test/integration/session-status.test.ts` (no session → error; init → status reports active; second init refused; destructive-counts sidecar persists across simulated server-restart with same configDir while in-memory session is gone per §2.2). Tool-list assertions in `test/integration/login.test.ts` + `dynamic-tools.test.ts` updated for the new tool. `npm run check` green (767 → 792 tests, +25 new). Once merged, advance to W2 Day 1. |
...
- [x] **W1 Day 5** — `session_status` + persisted destructive counter _(this PR)_
...
- **ADR-006** — `userOid = idTokenClaims.oid` _(this PR — codifies decision 15 with the actual code change that surfaces `idTokenClaims.oid` via `Authenticator.accountInfo`)_.
