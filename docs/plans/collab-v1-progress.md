...
Last updated: W1 Day 4 — multi-root-md handling landed
Current milestone: W1 Day 4 — Multi-root-md handling + `16-multiple-root-md.test.ts` (this PR)
Next milestone: W1 Day 5 — `session_status` + persisted destructive counter
...
| --------- | ---------------------------------------- | --------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| W1 Day 4  | `copilot/continue-collab-implementation` | _this PR_ | 2026-04-19 | Multi-root-md handling landed. `runInitProject` now opens a second browser picker (W0 form-factory slot, slot URL re-pointed) for the authoritative `.md` file selection — shown for every folder with ≥1 root `.md` files (zero-`.md` continues to throw `NoMarkdownFileError`; the multi-md interim "future version" message is gone). Picker option-set check rejects any submitted id that is not in the file list. Tool description + opening message updated to reflect the two-form flow. New `test/integration/16-multiple-root-md.test.ts` covers N=1 (form pre-selects, spy confirms), N=3 (empty submit → 400, then real selection; siblings unmodified), and a smuggle test (foreign drive item id rejected by option-set check). `test/integration/01-init-write-read-list.test.ts` `pickerSpy` extended to chain folder → file selections; existing 3 rows still green. `npm run check` green (764 → 767 tests, +3 new). Once merged, advance to W1 Day 5. |
...
- [x] **W1 Day 4** — Multi-root-md handling (`16-multiple-root-md.test.ts`) _(this PR)_
